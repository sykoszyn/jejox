...
import { createHeartRateReading } from './actions';
...
export function HeartRateForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createHeartRateReading({
        value,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6" noValidate>
      ...
      <LargeButton type="submit" size="xl" fullWidth loading={pending} icon={<Activity size={22} />}>
        Guardar pulso
      </LargeButton>
    </form>
  );
