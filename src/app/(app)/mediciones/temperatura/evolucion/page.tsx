...
export default async function EvolucionTemperaturaPage({
  ...
  const points = (data ?? []).map((r) => ({ timestamp: r.measured_at, temperatura: r.value }));

  return (
    <div>
      <PageHeader title="Evolución de temperatura" backHref="/mediciones" />
      <div className="px-4 py-6 max-w-2xl mx-auto flex flex-col gap-6">
        <RangeFilter basePath="/mediciones/temperatura/evolucion" selected={days} />
        {points.length === 0 ? (
          <EmptyState
            icon={<TrendingUp size={40} />}
            title="Todavía no hay registros de temperatura"
            description="Cuando registres mediciones vas a ver el gráfico acá."
          />
        ) : (
          <EvolutionChart
            data={points}
            series={[{ key: 'temperatura', label: 'Temperatura', color: '#0f6e58' }]}
            unit={data?.[0]?.unit ?? '°C'}
          />
        )}
      </div>
